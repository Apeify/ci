...
#                    must not overwrite. NO DEFAULT, and commented out below on
#                    purpose - it is the only guard against --delete removing
#                    what the server owns under app/, and the right list is
#                    different for every site, so it has to be written rather
#                    than inherited.
#
    ...
    steps:
      ...
      - uses: Apeify/ci/actions/deploy@075bf4b34f1200caef06a26aee50de5bb8e3fc76 # v2.1.0
        ...
        with:
          ...
          # Server-managed state and server-only config that the deploy must
          # never overwrite. COMMENTED OUT ON PURPOSE, and left that way unless
          # you have something to list: there is no default, because the right
          # answer is different for every site.
          #
          # This is the ONLY thing standing between `rsync --delete` and
          # whatever the server owns under app/. Anything there that the repo
          # does not also contain is removed on the next deploy unless it is
          # named here - so write this list from YOUR server's layout. The two
          # lines below are one site's answer, shown to make the shape obvious,
          # not a starting set to inherit.
          #
          # One pattern per line. Every list here is newline-separated, never
    ...
    steps:
      ...
      - uses: Apeify/ci/actions/deploy@075bf4b34f1200caef06a26aee50de5bb8e3fc76 # v2.1.0
        with:
          ...
          port: ${{ secrets.DEPLOY_PORT }}
          host-key: ${{ secrets.DEPLOY_HOST_KEY }}
          # MIRROR whatever deploy.yml sets here, not just its secrets and
          # variables. These are stub-level, so nothing carries them over for
          # you, and a DR deploy missing them runs --delete against
          # server-managed state on the failover host - the one place it is
          # least recoverable.
          #
          # Commented out to match deploy.yml, which ships with none because
          # the right list differs per site. If you filled it in there, fill in
          # the SAME list here.
          #
          # app-excludes: |
          #   cache/
